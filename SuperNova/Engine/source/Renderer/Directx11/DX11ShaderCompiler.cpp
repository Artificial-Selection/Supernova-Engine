...
ID3DBlob* CompileShader(const std::string& shaderStageSource, const char* shaderName, const char* shaderTarget)
{
...
{
    m_separateStreams = shaderDesc.Name != "Engine/ImGui";
...
std::vector<D3D11_INPUT_ELEMENT_DESC> DX11ShaderCompiler::GetInputLayoutDesc() const
{
    ...
    for (ui32 i = 0; i < d3dShaderDesc.InputParameters; i++)
    {
        ...
        D3D11_INPUT_ELEMENT_DESC d3dInputElementDesc = {
            ...
            .Format               = dx11_ComponentTypeToFormat(d3dSignatureParameterDesc.Mask, d3dSignatureParameterDesc.ComponentType),
            .InputSlot            = m_separateStreams ? i : 0,
            .AlignedByteOffset    = m_separateStreams ? 0 : D3D11_APPEND_ALIGNED_ELEMENT,
